...
async def upload_file(
    ...
    Args:
        file: 上传的文件
        session_id: 可选，关联的会话 ID（通知 LLM 文档已上传）
    ...
    try:
        ...
        try:
            ...
            vector_index_service.index_single_file(str(file_path), user_id=str(user.get("user_id", "")))
            logger.info(f"向量索引创建成功: {file_path}")
        except Exception as e:
            logger.error(f"向量索引创建失败: {file_path}, 错误: {e}")
            # 注意：即使索引失败，文件上传仍然成功，只是记录错误日志

        # 6. 如果传了 session_id，通知会话文档已上传，方便 LLM 后续检索
        if session_id:
            try:
                from app.services.rag_agent_service import rag_agent_service
                await rag_agent_service.memory_store.append_message(
                    session_id=session_id,
                    role="assistant",
                    content=f"📄 文档《{safe_filename}》已上传到知识库。"
                            f"当你问我文档中的内容时，我会使用知识检索工具来查找相关信息。",
                )
                logger.info(f"已通知会话 {session_id} 文档 {safe_filename} 上传完成")
            except Exception as e:
                logger.warning(f"通知会话失败: {e}")
