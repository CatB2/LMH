...
class VectorIndexService:
    ...
    def _extract_text_from_pdf(path: Path) -> str:
        ...
        if not HAS_PDF_SUPPORT:
            raise RuntimeError("PDF 支持需要安装 pypdf: pip install pypdf")
    ...
    def index_single_file(self, file_path: str, user_id: str = ""):
        """
        ...
        try:
            ...
            logger.info(f"读取文件: {path.name}, 内容长度: {len(content)} 字符")
            ...
            # 3. 使用文档分割器
            documents = document_splitter_service.split_document(content, normalized_path, user_id)
            logger.info(f"文档分割完成: {file_path} -> {len(documents)} 个分片")

            # 4. 添加文档到向量存储（带重试）
            if documents:
                self._add_documents_with_retry(documents, file_path)
    ...
    def _add_documents_with_retry(self, documents, file_path: str, max_retries: int = 2):
        """带重试的文档添加操作"""
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"添加文档重试 {attempt + 1}/{max_retries}: {e}")
                    time.sleep(1)
                else:
                    raise
