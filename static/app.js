...
class SuperBizAgentApp {
    constructor() {
        ...
        this.currentModel = localStorage.getItem('currentModel') || 'qwen-max';
        ...
        this.updateUI();
        // 初始化模型显示
        const modelBtn = document.getElementById('currentModelText');
        if (modelBtn) modelBtn.textContent = this.currentModel;

        // 检查认证状态
        if (this.accessToken) {
            this.hideAuth();
            this.loadUsername();
            this.fetchServerSessions(); // 登录态下拉取服务端会话
        } else {
            this.showAuth();
        }
    ...
    async _authFetchJSON(url, options = {}) {
        const response = await this._authFetch(url, options);
    ...
    async handleLogin() {
        ...
        try {
            ...
            const data = await resp.json();
            if (!resp.ok) {
                this.setAuthError(data.detail || '登录失败');
                btn.disabled = false;
                btn.textContent = '登录';
                return;
            }
            ...
            this.accessToken = data.data?.access_token || data.access_token;
            this.refreshToken = data.data?.refresh_token || data.refresh_token;
            ...
            this.hideAuth();
            this.updateLogoutBtn();
            await this.fetchServerSessions();
            this.newChat();
        } catch (e) {
            this.setAuthError('网络错误，请检查服务是否运行');
        } finally {
            btn.disabled = false;
            btn.textContent = '登录';
        }
    ...
    async handleRegister() {
        ...
        if (!username || !password) {
            this.setRegisterError('请输入用户名和密码');
            return;
        }
        if (password.length < 6) {
            this.setRegisterError('密码至少6位');
            return;
        }
        if (password !== confirm) {
            this.setRegisterError('两次密码输入不一致');
            return;
        }
        ...
        try {
            ...
            const data = await resp.json();
            if (!resp.ok) {
                this.setRegisterError(data.detail || '注册失败');
                btn.disabled = false;
                btn.textContent = '注册';
                return;
            }
            ...
            this.setRegisterError('网络错误，请检查服务是否运行');
        } finally {
            btn.disabled = false;
            btn.textContent = '注册';
        }
    ...
    updateLogoutBtn() {
        ...
        if (btnText && this.username) {
            btnText.textContent = `${this.username} · 退出`;
        } else if (btnText) {
    ...
    bindEvents() {
        ...
        if (this.modelSelectorBtn) {
            this.modelSelectorBtn.addEventListener('click', (e) => { e.stopPropagation(); this.toggleModelDropdown(); });
            const modelItems = document.querySelectorAll('#modelDropdown .dropdown-item');
            modelItems.forEach(item => { item.addEventListener('click', () => { this.selectModel(item.dataset.model); }); });
        }
        // 模式选择下拉菜单
    ...
    async fetchServerSessions() {
        try {
            console.log('[SESSIONS] 开始拉取服务端会话');
            const resp = await this._authFetch(`${this.apiBaseUrl}/sessions`);
            const result = await resp.json();
            console.log('[SESSIONS] 响应:', result.code, result.data?.sessions?.length || 0, '个会话');
            ...
            for (const s of serverSessions) {
                if (!localIds.has(s.session_id)) {
                    // 用首条消息的前30字作标题，没有则用session_id
                    const title = s.first_message
                        ? s.first_message.substring(0, 30) + (s.first_message.length > 30 ? '...' : '')
                        : s.session_id.substring(0, 30);
    ...
    renderChatHistory() {
        if (!this.chatHistoryList) {
            console.warn('[RENDER] chatHistoryList 为 null');
            return;
        }

        console.log('[RENDER] 渲染会话列表:', this.chatHistories.length, '个');
        this.chatHistoryList.innerHTML = '';

        if (this.chatHistories.length === 0) {
            console.log('[RENDER] 会话列表为空');
            return;
        }
    ...
    async loadChatHistory(historyId) {
        ...
        try {
            // 从后端获取会话历史
            const response = await this._authFetch(`${this.apiBaseUrl}/chat/session/${historyId}`);
            if (response.ok) {
                const data = await response.json();
                const backendHistory = data.data?.history || data.history || [];
    ...
    async deleteChatHistory(historyId) {
        try {
            // 调用后端API清空会话
            const response = await this._authFetch(`${this.apiBaseUrl}/chat/clear`, {
                method: 'POST',
                headers: {
    ...
    selectModel(model) {
        ...
        const btn = document.getElementById('currentModelText');
        if (btn) btn.textContent = model;
        // 更新下拉选项 active 状态
        const items = document.querySelectorAll('#modelDropdown .dropdown-item');
        items.forEach(i => { i.classList.toggle('active', i.dataset.model === model); });
        document.getElementById('modelDropdown').classList.remove('show');
        this.showNotification('已切换到: ' + model, 'info');
    }
    // 切换模式下拉菜单
    toggleModeDropdown() {
    ...
    async sendMessage() {
        ...
        try {
            ...
            if (this.isCurrentChatFromHistory && this.currentChatHistory.length > 0) {
                this.updateCurrentChatHistory();
                this.renderChatHistory();
            } else if (!this.isCurrentChatFromHistory && this.currentChatHistory.length > 0) {
                // 新对话第一次收到回复后，保存到本地历史
                const existingIdx = this.chatHistories.findIndex(h => h.id === this.sessionId);
                if (existingIdx === -1) {
                    const firstUserMsg = this.currentChatHistory.find(msg => msg.type === 'user');
                    this.chatHistories.unshift({
                        id: this.sessionId,
                        title: firstUserMsg ? firstUserMsg.content.substring(0, 30) : '新对话',
                        messages: [...this.currentChatHistory],
                        createdAt: new Date().toISOString(),
                        updatedAt: new Date().toISOString(),
                    });
    ...
    handleBackendError(error) {
        const message = error?.message || '发送消息时出现错误';
        const normalized = message.toString();
        if (normalized.includes('另起一个新会话') || normalized.includes('请另起一个新会话') || normalized.includes('超过限制')) {
            this.showNotification('当前会话历史已达到限制，请新建一个对话继续使用。', 'warning');
        } else {
            this.showNotification(message, 'error');
        }
        this.addMessage('assistant', '抱歉，发送消息时出现错误：' + message);
    }
    ...
    async sendQuickMessage(message) {
        ...
        try {
            const response = await this._authFetch(`${this.apiBaseUrl}/chat`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (data.code === 200 || data.message === 'success') {
                ...
                if (chatResponse && chatResponse.success) {
                    ...
                    this.addMessage('assistant', answer, false, true, chatResponse.source_files);
    ...
    async sendStreamMessage(message) {
        try {
            const response = await this._authFetch(`${this.apiBaseUrl}/chat_stream`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    // 添加消息到聊天界面
    addMessage(type, content, isStreaming = false, saveToHistory = true, sourceFiles = null) {
        ...
        if (sourceFiles && sourceFiles.length > 0) {
            ...
            sourceDiv.className = 'message-source-files';
            const label = document.createElement('span');
            label.className = 'source-files-label';
            label.textContent = '来源: ';
            sourceDiv.appendChild(label);
            sourceFiles.forEach((f, i) => {
                const tag = document.createElement('span');
                tag.className = 'source-file-tag';
                tag.textContent = f;
    ...
    async uploadFile(file) {
        ...
        try {
            ...
            // 发送上传请求（FormData 不能用 JSON Content-Type）
            const uploadHeaders = {};
            if (this.accessToken) {
    ...
    async sendAIOpsRequest(loadingMessageElement) {
        try {
            const response = await this._authFetch(`${this.apiBaseUrl}/aiops`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    updateAIOpsStreamContent(messageElement, content) {
        ...
        if (messageContentWrapper) {
            ...
            messageContent.innerHTML = this.renderMarkdown(formatted);
            this.highlightCodeBlocks(messageContent);
            this.scrollToBottom();
        }
    }
